...
local ConfirmDialong = Class(WindowNodeClass)

-- ### 表示这个窗口只能同时弹出1个
windowUtility.SetMutex(ConfirmDialong, true)

function ConfirmDialong:Ctor()
    self.confirmCallback = LuaDelegate.New()
    self.cancelCallback = LuaDelegate.New()
end

function ConfirmDialong:OnInit()
    -- 加载 登录界面
...
function ConfirmDialong:GetRootHangingPoint()
    return self:GetUIManager():GetDialogLayer()
end

function ConfirmDialong:OnWillShow(text, table, confirmFunc, cancelFunc ,args)
    self.msgText = text
    print(type(table))
...
function ConfirmDialong:OnComponentReady()
...
function ConfirmDialong:OnResume()
    ConfirmDialong.base.OnResume(self)
    -- 设置文字
...
function ConfirmDialong:OnPause()
    ConfirmDialong.base.OnPause(self)
--    print('OnPause')
    self:UnregisterControlEvents()
end

function ConfirmDialong:RegisterControlEvents()
    -- 注册 Button 的事件
...
function ConfirmDialong:UnregisterControlEvents()
    -- 取消注册 Button 的事件
...
function ConfirmDialong:OnButtonClicked()
    self.cancelCallback:Invoke()
    self:Hide(true)
end

function ConfirmDialong:OnConfirmButtonClicked()
...
--- 动画相关
-----------------------------------------------------------------------
function ConfirmDialong:IsTransition()
    return true
end

-- ## 在这里执行 淡出函数! (immediately 值只针对 WindowNode 及其子类)
function ConfirmDialong:OnExitTransitionDidStart(immediately)
    ConfirmDialong.base.OnExitTransitionDidStart(self, immediately)
