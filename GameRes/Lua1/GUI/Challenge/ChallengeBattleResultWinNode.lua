...
function ChallengeBattleResultWinNode:Show(msg)
    -- -- # 等级
    print("+++++++++++++++++++++++    ",#msg.awardItems,msg.awardItems,msg.systemID,msg.success,msg.fbID,msg.fbID,msg.fbID)
    local color
    for i = 1, #msg.awardItems do
        color=nil
        if msg.awardItems[i].itemColor ~=0 then
            color=msg.awardItems[i].itemColor
        end
    print(msg.awardItems[i].itemID,msg.awardItems[i].itemNum,msg.awardItems[i].itemColor)
    local item = require 'GUI.Challenge.ChallengeGeneralItem'.New(self.Layout,msg.awardItems[i].itemID,msg.awardItems[i].itemNum,color)
