...
function SelectChallengdungeonCls:InitControls()
	...
	--剩余挑战次数
	self.lastText = transform:Find('SelectDifficultyBox/StageTitle/Text'):GetComponent(typeof(UnityEngine.UI.Text))
	--文字提示
	...
	self.TiLiText = transform:Find('SelectChallengdungeonFightButton/Text'):GetComponent(typeof(UnityEngine.UI.Text))
...
function SelectChallengdungeonCls:InitView()
	...
	self:ChildClickedCallBack(self.Chapter[self.firstChapterIndex])
end
-- ------------改变透明度
...
-- end
 ...
 function SelectChallengdungeonCls:ChildClickedCallBack(table)
	...
	for i=1,#self.childData.BossPortrait do
		hzj_print(self.childData.BossPortrait[i][num-1],self.difficults[i].difficultImage)
	end
	...
	local id = self.currentClickBut.index+self.childData.data:GetFirstLevelID()-1	
	local needTili = require"StaticData.ChapterLevel":GetData(id):GetVigorToConsume()
...
-----足够的体力
	...
	if self.childData.ChapterInfoID==1003 then
		local teamsId = {}
		for i=1,#enemyTeams do
			teamsId[#teamsId+1]=enemyTeams[i]:GetId()+index
	 			
		end


		hzj_print("teamsId[1]",teamsId[1],index)

		allTeams = BattleUtility.CreateBattleTeamsByIDs(chapterLevelData:GetMonsterLevel(),chapterLevelData:GetMonsterStage(),teamsId[1])
	else
		allTeams = BattleUtility.CreateBattleTeamsByLevelID(id)
	end
...
function SelectChallengdungeonCls:StartChallengFight()
	...
	if self.childData.ChapterInfoID==1003 then
		local gameTool = require "Utils.GameTools"
		local index = gameTool.AutoWeedDay()
		...
		hzj_print("teamsId[1]",teamsId[1])

		foeTeams = BattleUtility.CreateBattleTeamsByIDs(chapterLevelData:GetMonsterLevel(),chapterLevelData:GetMonsterStage(),teamsId[1])
	else
		foeTeams = BattleUtility.CreateBattleTeamsByLevelID(id)
	end
	...
	battleParams:SetSceneID(chapterLevelData:GetSceneID())
...
function SelectChallengdungeonCls:ExploreMapQueryResult(msg)
	...
	if self.requestMapQuery == false then
		if msg.systemID==self.childData.ChapterInfoID then		
			self.lastText.text="(剩余次数".. msg.remainCount..")"
		end
		return
	end
	self.requestMapQuery = false
	...
	if msg.systemID==self.childData.ChapterInfoID then
		...
		self.lastText.text="(剩余次数".. remainCount..")"
...
function SelectChallengdungeonCls:OnSelectChallengdungeonReturnButton__1_Clicked()
	...
	-- --探险系统引导
	-- utility.GetGame():GetSystemGuideManager():SetNeetSystemGuideID(kSystem_Guide[4].systemGuideID,self)
	-- utility.GetGame():GetSystemGuideManager():SetNeetSystemGuideID(kSystem_Guide[7].systemGuideID,self)
	-- utility.GetGame():GetSystemGuideManager():SetNeetSystemGuideID(kSystem_Guide[8].systemGuideID,self)
	-- utility.GetGame():GetSystemGuideManager():SetNeetSystemGuideID(kSystem_Guide[9].systemGuideID,self)
