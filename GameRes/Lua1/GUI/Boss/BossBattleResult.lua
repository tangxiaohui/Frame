...
local BossBattleResult = Class(BaseNodeClass)
windowUtility.SetMutex(BossBattleResult, true)

function BossBattleResult:Ctor()
end
...
--- 场景状态
-----------------------------------------------------------------------
function BossBattleResult:OnInit()
	-- 加载界面(只走一次)
	utility.LoadNewGameObjectAsync('UI/Prefabs/BossBattleResult', function(go)
...
local function InitControls(self)
    ...
    -- 胜利结果节点 --
    local BattleResultWinNodeClass = require "GUI.Boss.BossBattleResultWinNode"
    ...
    self.dataButtonObject = self.dataButton.gameObject
     self.dataButtonObject.gameObject:SetActive(false)
...
function BossBattleResult:GetRootHangingPoint()
    return self:GetUIManager():GetModuleLayer()
end

function BossBattleResult:OnComponentReady()
	-- 界面加载完毕 初始化函数(只走一次)
	InitControls(self)
    InitLogic(self)
end
...
function BossBattleResult:OnResume()
    BossBattleResult.base.OnResume(self)
...
function BossBattleResult:OnPause()
    BossBattleResult.base.OnPause(self)
