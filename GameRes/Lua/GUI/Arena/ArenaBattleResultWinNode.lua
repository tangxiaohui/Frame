...
local function InitControls(self)
    ...
    -- 历史最高
    self.BestRankLabel = transform:Find('BestRank/BestRankLabel'):GetComponent(typeof(UnityEngine.UI.Text))
    -- 当前排名
...
function BattleResultWinNode:Show(msg)
    -- -- # 等级
    print("竞技场战斗结果>>>>>>>>>>>>>",msg.curRank,msg.historyAward,msg.killHighLevelAward)
    self.BestRankLabel.text = msg.historyHighRank
    self.curRankLabel.text = msg.curRank
    local uprank = msg.lastRank - msg.curRank
    self.upRankLabel.text = uprank
