local ExploreSystemGuide = Class(LuaObject)

function ExploreSystemGuide:Ctor(id,tables)
	self.id=id
	self.tables=tables
    	hzj_print("ExploreSystemGuide:Ctor(id)",self.tables.itemsChild[1].ChallengeButton)
   	self:InitSystemGuideStep()
   	self.currenStep=nil
end
function ExploreSystemGuide:InitSystemGuideStep()
	...
	self.step[1].stepInfo="出海的船只已经准备就绪，我们可以随时登船探险啦！"
	...
	self.step[2].stepButton=self.tables.Button
	self.step[2].stepPortraitPos=2
	self.step[2].stepInfo="每次探险都会随机获得不同的奖励，团队等级32级后还有概率可以发现巨龙的宝藏呢！"
...
function ExploreSystemGuide:GetSystemGuideNextStep()
