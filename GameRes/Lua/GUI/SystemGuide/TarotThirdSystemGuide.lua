local TarotThirdSystemGuide = Class(LuaObject)

function TarotThirdSystemGuide:Ctor(id,tables)
	self.id=id
	self.tables=tables
    	hzj_print("TarotThirdSystemGuide:Ctor(id)")
   	self:InitSystemGuideStep()
   	self.currenStep=nil
end
function TarotThirdSystemGuide:InitSystemGuideStep()
	...
	self.step[1].stepType=2
	self.step[1].stepButton=self.tables.activeButton
	self.step[1].stepPortraitPos=1
	self.step[1].stepInfo="点击按钮"
	...
	-- self.step[2].stepButton=self.tables.activeButton
...
function TarotThirdSystemGuide:GetSystemGuideNextStep()
