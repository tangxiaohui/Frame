...
local BossBattleResultWinNode = Class(BaseNodeClass)
...
function BossBattleResultWinNode:Ctor(transform)
    print(">>>>>>>>>>>>>>>>  is me ")
    self:BindComponent(transform.gameObject, false)
    InitControls(self)
end

function BossBattleResultWinNode:OnResume()
end

function BossBattleResultWinNode:OnPause()
end

-- TODO: 暂时定为FBResponse
function BossBattleResultWinNode:Show(msg)
      for i=1,#msg.award do
            debug_print(msg.award[i].itemID)
            self:AddChild(require"GUI.Boss.BossAwardItem".New(self.Layout,msg.award[i].itemID,msg.award[i].itemNum,msg.award[i].itemColor))
    end
    -- -- # 等级
...
function BossBattleResultWinNode:Close()
    -- ### 隐藏 ### --
...
return BossBattleResultWinNode